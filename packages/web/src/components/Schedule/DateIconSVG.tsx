...
export const DateIconInstance: React.FC<DateIconProps> = ({
    date,
    ...props
}) => {
    ...
    return (
        <svg
            ...
            viewBox="0 0 600 600"
        >
            <title>Date Icon Instance</title>
            ...
            {calendarArray.map((element, index) => {
                ...
                if (isSameMonth(date, element)) {
                    const className = isSameDay(date, element)
                        ? 'active'
                        : 'inactive';
                    return (
                        <rect
                            key={element.toISOString()}
                            className={className}
                            stroke="none"
                            x={x}
                            y={y}
