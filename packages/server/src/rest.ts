...
const mapSearchFields =
	...
	(token: string) => {
		return searchableFields.map((field) => {
			const name =
				typeof entity === "string" ? entity : (entity as EntityClass<R>).name;
...
interface CrudActions<I extends NonNullable<Primary<R>>, R extends object> {
	...
	search:
		| ((params: SearchParams, opts: RequestResponse) => Promise<ListReturn<R>>)
		| null;
}

interface FilterOptions<R extends object> {
...
const crud = <I extends NonNullable<Primary<R>>, R extends object>(
	path: string,
	...
	if (actions.update) {
		router.put(`${path}/:id`, async (req, res, next) => {
			try {
				if (actions.update) {
					const record = await actions.update(req.params.id as I, req.body, {
						req,
						res,
					});
					res.json(record);
		...
		try {
			...
			const data = pricesAndProducts.map((prod) => {
				...
				return {
					...
					images:
						images.length !== 0
							? images.map((v) => v.replace(stripeClient.THUMBNAIL_STATIC, ""))
							: undefined,
